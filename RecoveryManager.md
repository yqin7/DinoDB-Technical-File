...
+----------------------------+
| 用户输入命令                 |
| insert 1 10 into test      |
+------------+---------------+
             |
             v
+------------+---------------+
| 客户端发送TCP请求            | [main.go - client部分]
| 服务器接收并分配clientId     | [main.go - startServer]
+------------+---------------+
             |
             v
+------------+---------------+
| REPL接收并解析命令           | [repl.go - REPL.Run方法]
| 1. scanner.Scan()          |
| 2. 提取触发词"insert"       |
| 3. 查找对应处理函数          |
+------------+---------------+
             ...
             v
+------------+---------------+
| 调用处理函数                 | [recovery_repl.go - 匿名函数调用]
| HandleInsert(db, tm, rm,   | 
| payload, clientId)         |
+------------+---------------+
             |
             v
+------------+---------------+
| 参数解析和验证               | [recovery_repl.go - HandleInsert函数内]
| key=1, value=10, table=test|
+------------+---------------+
             |
             v
[recovery_repl.go - HandleInsert函数内]
+-------------------------------+     +----------------------+
| 检查键是否已存在                |     | 键已存在             |
| table.Find(key)              +---->| 返回错误             |
+---------------+---------------+     +----------------------+
...
+-------------------------------+     +----------------------+
| 写入预写日志(WAL)              |     | 日志写入失败          |
| rm.Edit(clientId,table,INSERT)|---->| 返回错误             |
+---------------+---------------+     +----------------------+
                |
                | 日志写入成功
                v
...
+-------------------------------+     +----------------------+
| 执行实际的插入操作              |     | 插入失败             |
| concurrency.HandleInsert()   +---->| 1.写日志标记no-op     |
+---------------+---------------+     | 2.移除txStack中记录  |
                |                     | 3.回滚事务           |
                | 插入成功             +----------+----------+
                v                                |
