...
- **未提交事务也需要刷盘：**
  ...
  - 如果一个数据页同时包含已提交和未提交的修改，如果策略是未提交事务不刷盘，调用checkpoint时就不能直接将整页刷盘，必须创建额外的"干净版本"数据页：只包含已提交事务的修改和未提交事务修改之前的数据。由于数据库I/O是以页为单位的，这种做法会导致大量额外的内存复制和磁盘写入操作，严重影响性能。
...
- 创建空的活跃事务ID列表
- 遍历 txStack 哈希表，收集所有未提交事务的ID，目的是为了在checkpoint的日志记录所有活跃事务ID。检查点记录了所有活跃事务ID后，当进行崩溃后活跃事务扫描的时候，**获得崩溃时活跃事务的公式是：崩溃时活跃事务 = 最近检查点时的活跃事务 - 最近检查点后已提交的事务 + 检查点后新开始的事务。**换一种说法，系统不需要进行全日志扫描确定崩溃时的活跃事务了，**只需要在检查点开始向后扫描日志就可以获得崩溃时的活跃事务了**。这样大大降低了扫描的成本。
...
- MySQLInnoDB实现了ARIES协议中的**补偿日志记录**(Compensation Log Record)概念：
