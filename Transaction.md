...
+------------+---------------+
             ...
             v
+------------+---------------+
| 调用处理函数                 | [transaction_repl.go - 匿名函数调用]
| HandleFind(db, tm,         | 
| payload, clientId)         |
+------------+---------------+
             |
             v
+------------+---------------+
| 参数解析和验证               | [transaction_repl.go - HandleFind函数内]
| key=1, table=test          |
+------------+---------------+
             |
             v
...
+-------------------------------+     +----------------------+
| 获取资源锁                      |     | 检测到死锁            |
| tm.Lock(clientId,table,key,R) +---->| 执行事务回滚          | [transaction_repl.go - HandleFind函数内]
+---------------+---------------+     +----------+----------+
                |                                |
                | 成功                            |
                v                                |
...
+-------------------------------+                |
                ...
                v
+---------------+---------------+
| 事务管理                       | [main.go - handleConn函数中的defer]
| 1. 锁保持到事务结束              | [通过defer tm.Commit(clientId)]
| 2. 事务提交或连接关闭时释放锁     |
+-------------------------------+
```
