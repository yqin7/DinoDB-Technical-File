...
- **增长阶段**：**事务只能获取锁，不能释放任何锁，不需要在事务一开始获取全部所需的资源锁**
...
## 1.2 严格两阶段锁定 LOCK API原理体现

1. 在事务管理器中，Lock的目的为指定的数据库索引的键上读锁或者写锁。这个方法遵循 strict 2PL 协议，确保事务间的隔离性。

2. 实现过程主要分为几个关键步骤：首先，通过客户端 ID 获取对应的事务对象。接着检查该事务是否已持有该资源的锁 - 如果持有相同类型的锁，就直接返回；如果尝试将读锁升级为写锁，会拒绝这种操作，因为 strict 2PL 不允许锁升级。

3. 然后是冲突检测和死锁预防。我们会寻找所有与当前锁请求冲突的事务，并在等待图中添加相应的等待边，这样可以跟踪事务间的依赖关系。接着用 DFS或者拓扑排序 算法检测等待图中是否存在环，如果发现死锁，就回滚当前事务。

4. 如果没有死锁，就获取资源的实际锁 - 对于读锁，只有在没有写锁时才能获取；对于写锁，需要资源完全空闲才能获取。如果没有满足上述要求，当前事务就会被阻塞等待。最后更新事务的状态，记录它现在持有的锁资源。

5. 这种设计既满足了事务的隔离性要求，符合两个阶段和严格特性的工作原理。

## 1.3 Strict 2PL如何保证ACID特性
...
## 1.4 严格两阶段锁定协议 vs 普通两阶段锁定协议：关键区别
...
- **如果当前客户端已经持有读锁，根据strict 2PL的约束，不可以在执行事务时升级锁。否则会返回错误（之后会更改成回滚事务）**
