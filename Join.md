...
1. 对左右两个表使用相同的哈希函数，构建左右两个哈希索引，将左右两表相同的键划分进左右两哈希索引相同编号的桶。后面只需在对应桶之间进行匹配，既减少了比较范围，也方便并行计算和内存优化。


**2. 探测阶段**

1. 接下来获取左右表的所有哈希桶(buckets)，对应位置的两个桶可能包含匹配的记录。
...
- 本项目中布隆过滤器大小为1024位，布隆过滤器误差计算公式如下

  ![公式](https://latex.codecogs.com/svg.latex?p%20\approx%20\left(1%20-%20e^{-\frac{kn}{m}}\right)^k)

​	`p`：误判率（false positive probability）
